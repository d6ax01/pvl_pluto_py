...
def Convert_RawToCSV(filePath, folderPath, prefix, countIndex, frequency, cellLine, position):
    ...
    path = folderPath
    depth_file = f'%s_%s_%s%s' % (position, prefix, countIndex, f'_ALL0_f1_nshfl.raw')  # 심미안 뷰어에서 'Decode' - 'ToF' 가 선택이 되어야한다.
    intensity_file = f'%s_%s_%s%s' % (position, prefix, countIndex, f'_ALL0_f1_nshfl_VC(2)_DT(Raw8).raw')
    width = 320
    height = 240
    ...
    DepthCSV = f'%s_%s%s.csv' % (position, f'depth', countIndex)
    ...
    IntensityCSV = f'%s_%s%s.csv' % (position, f'intensity', countIndex)
    ...
    CTValue = depth[114:124, 154:164]
    CTValueRavel = np.ravel(CTValue, order='C')
    remove_set = {0}
    CTValueSort = [i for i in CTValueRavel if i not in remove_set]
    CTValueAvg = np.mean(CTValueSort)


    # 뎁스 오차, 노이즈, 세기 대표 값 계산
    if position == 300:  # 측정 위치 300 mm
        # Depth Error
        gResult[(countIndex + ((cellLine - 1) * gCaptureCnt)), 0] = CTValueAvg #depth[114:124, 154:164].mean()  # CT
    else:  # 측정 위치 500 mm
        # Depth Error
        gResult[(countIndex + ((cellLine - 1) * gCaptureCnt)), 1] = CTValueAvg #depth[114:124, 154:164].mean()  # CT
...
def validation(prefix, path, frequency, mode, count, pos):
    ...
    for i in range(0, count):
        filePath = r'{0}\{1}_{2}_{3}.raw'.format(path, pos, prefix, i)  # ++_ALL0_f1_nshfl_VC(2)_DT(Raw8).raw
        result = simmian.Capture(filePath, captureCount=1)
        # msg_print(filePath)
        if frequency == 100:
...
def ProcCreateSetFile100():
    ...
    if bfile == True:
        ...
        msg_print(f'100 MHz 셋파일을 찾지못했습니다.')
        return False
...
def ProcSaveRaw100mhz30cm(mode, position):
    msg_print(
    ...
    gOutputFilePrefix = f'image100mhz'
    validation(gOutputFilePrefix, savePath, 100, mode, gCaptureCnt, position)
...
def ProcSaveRaw20mhz30cm(mode, position):
    ...
    savePath = g20mhz30cm
    gOutputFilePrefix = f'image20mhz'
    validation(gOutputFilePrefix, savePath, 20, mode, gCaptureCnt, position)
...
def InitGlobalOffsetWriteI2C():
    ...
    set_board_offset_f2 = 16660  # 계산한 옵셋이 0xabcd 이라면, WriteI2C 에 입력되는 값은 0xcdab 로 넣어야한다.
    set_board_offset_f2_ = TestReversed(set_board_offset_f2)
    str = (
...
def Measurement(frequency, distance, motorValue):
    ...
    if frequency == 100:
        ...
        time.sleep(1)
        ProcSaveRaw100mhz30cm(1, distance)  # 저장
        ProcSimmianReset()
    else:
        ProcLoadSetFile20mhz()  # 셋파일 로딩
        time.sleep(1)
        ProcSaveRaw20mhz30cm(1, distance)  # 저장
        ProcSimmianReset()


def MotorTest(frequency, distance, motorValue):
    ProcSimmianReset()  # 리셋
    motor_move(distance=distance, motion_abs=motorValue)  # 이동
    input("Press Key")
...
if __name__ == "__main__":
    ...
    if (want_home_position_at_start):
        ...
        motor_move(distance=500, motion_abs=399.7)  # 2022_11_30, 지그교체
        time.sleep(0.5)
    ...
    Measurement(100, 500, 399.7) # 500 mm 에서 평가
    ProcCreateReport(1)  # 0 -> find global offset , 1 -> only measurement
    ...
    # 로딩 위치로 이동
    motor_move(distance=500, motion_abs=399.7)  # 이동
    time.sleep(1)
    ...
    Measurement(20, 500, 399.7)  # 500 mm 에서 평가
    ProcCreateReport(1)  # 0 -> find global offset , 1 -> only measurement
    ...
    # 모터 로딩으로 이동 및 닫기 ***************************************************************************************
    motor_move(distance=500, motion_abs=399.7)
    ProcMotorClose()
