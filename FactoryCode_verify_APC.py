...
gSaveModulePath = f' '
g100MHzSetFileName = f'[val]63D_EVT0p1_QVGA_A_C02_M1_R300_D60_SINGLE_100M_FLOOD_DPHY960_AE_MIPI960_cal_maskoff_CalOff_3J'
...
def APCCheck(freq, Value):
    ...
    if freq == 100:
        ...
        SetVcselCurrent(Value)
        PDValue = ReadPdValue()  # value = ReadPdValue()

        log_message = f'ReadPDValue({PDValue}   ), <- PD val 100MHz {Value}mA : ReadPDValue(HEX-> {PDValue} )'
        print(log_message)
        ...
        log_message = f'ReadPDValue({PDValue}   ), <- PD val 020MHz {Value}mA : ReadPDValue(HEX-> {PDValue} )'
        print(log_message)
...
def find_judge(mode, freq, data, indexCount):
    ...
    DepthErrorAVG300_Spec_Center = 24.0  # //300mm Depth Validation Data_f1 -> Depth mean of center ROI
    ...
    DepthErrorAVG500_Spec_06Field = 40.0;  # 500mm Depth Validation Data_f1 (QVGA) -> Depth mean of 0.6F ROI
    ...
    DepthNoiseAVG500_Spec_Center = 5.0;  # 500mm Depth Validation Data_f1 (QVGA) -> Validation Noise of center ROI
    ...
    if freq == 100:
        ...
        ConfidenceAVG_Spec_CenterLower = 938;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> Center ROI (하한선)
        ConfidenceAVG_Spec_CenterUpper = 1875;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> Center ROI (상한선)
        ConfidenceAVG_Spec_06FieldLower = 813;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> 0.6F ROI (하한선)
        ConfidenceAVG_Spec_06FieldUpper = 1625;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> 0.6F ROI (상한선)
    else:  # 20MHz
        # // 검사 사양 : 20MHz, 500mm VCSEL Optical Power Data_f2 (광량검사 데이터)
        ConfidenceAVG_Spec_CenterLower = 1000;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> Center ROI (하한선)
        ConfidenceAVG_Spec_CenterUpper = 2000;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> Center ROI (상한선)
        ConfidenceAVG_Spec_06FieldLower = 875;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> 0.6F ROI (하한선)
        ConfidenceAVG_Spec_06FieldUpper = 1750;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> 0.6F ROI (상한선)
    ...
    # // 시작 ******************************************************************************************************************************
    # // APC 검사 100MHz , lsw_fix1_f1 = 500 ~ 850 ,  lsw_fix2_f1 = 250 ~ 550
    # // APC 검사 20MHz ,  lsw_fix1_f2 = 600 ~ 1000 , lsw_fix2_f2 = 350 ~ 750
    ...
    if freq == 100:
        if gApc_check_at_lsw_fix1_f1 >= 480 and gApc_check_at_lsw_fix1_f1 <= 900:
            apc_lsw_fix1 = True
        else:
            apc_lsw_fix1 = False
        ...
        if gApc_check_at_lsw_fix2_f1 >= 250 and gApc_check_at_lsw_fix2_f1 <= 550:
            apc_lsw_fix2 = True
        else:
            apc_lsw_fix2 = False
        ...
        if gApc_check_at_lsw_fix1_f2 >= 600 and gApc_check_at_lsw_fix1_f2 <= 1020:
            apc_lsw_fix1 = True
        else:
            apc_lsw_fix1 = False
        ...
        if gApc_check_at_lsw_fix2_f2 >= 350 and gApc_check_at_lsw_fix2_f2 <= 750:
            apc_lsw_fix2 = True
        else:
            apc_lsw_fix2 = False
    ...
    if mode == 0:
        ...
        if (_b500mm_Depth_Validation_Data_f1_Depth_mean_of_01F_CT_ROI
            ...
            if freq == 100:
                msg_print(f'Error, Noise, Brightness, PD Value, 100MHz 평가 결과 -> PASS ')
            elif freq == 20:
                msg_print(f'Error, Noise, Brightness, PD Value, 20MHz 평가 결과 -> PASS ')
            else:
                msg_print('ERROR -> find_judge()')
            return 'PASS'
        else:
            if freq == 100:
                msg_print(f'Error, Noise, Brightness, PD Value, 100MHz 평가 결과 -> FAIL ')
            elif freq == 20:
                msg_print(f'Error, Noise, Brightness, PD Value, 20MHz 평가 결과 -> FAIL ')
            else:
                msg_print('ERROR -> find_judge()')
            return 'FAIL'
...
def Convert_RawToCSV(filePath, folderPath, prefix, countIndex, frequency, cellLine, position, setPDValue):
    # Convert_RawToCSV(filePath, path, prefix, i) #filePath RAW경로, path폴더경로, prefix파일이름, i파일이름순서, 측정위치
    global gResult, g100mhzDepthValue, g20mhzDepthValue, gIntecsityCT, gIntecsityLT, gIntecsityRT, gIntecsityRB, gIntecsityLB
    ...
    # save csv file
    DepthCSV = f'%s%s%s.csv' % (f'depth', countIndex, setPDValue)
    np.savetxt(os.path.join(folderPath, DepthCSV), depth, delimiter=',')
    # print(f'   Save : %s\%s', folderPath, DepthCSV)

    IntensityCSV = f'%s%s%s.csv' % (f'intensity', countIndex, setPDValue)
    ...
    if position == 300:  # 측정 위치 300 mm
        ...
        gIntecsityCT = intensity[114:124, 154:164].mean()  # CT
        gIntecsityLT = intensity[43:53, 59:69].mean()  # LT
        gIntecsityRT = intensity[43:53, 250:260].mean()  # RT
        gIntecsityRB = intensity[186:196, 250:260].mean()  # RB
        gIntecsityLB = intensity[186:196, 59:69].mean()  # LB
...
def validation(prefix, path, frequency, mode, count, pos, setPDValue):
    ...
    for i in range(0, count):
        ...
        # msg_print(filePath)
        if frequency == 100:
            Convert_RawToCSV(filePath, path, prefix, i, frequency, 1, pos, setPDValue)
            # RAW경로, 폴더경로, 파일이름, 파일이름순서, 동작주파수, , 측정위치 순이다.
        elif frequency == 20:
            Convert_RawToCSV(filePath, path, prefix, i, frequency, 2, pos, setPDValue)
            # RAW경로, 폴더경로, 파일이름, 파일이름순서, 동작주파수, , 측정위치 순이다.
        else:
            print('')
...
def ProcSaveRaw100mhz50cm(mode, setPDValue):
    ...
    validation(gOutputFilePrefix, savePath, 100, mode, gCaptureCnt, 500, setPDValue)
...
def ProcSaveRaw20mhz50cm(mode, setPDValue):
    ...
    validation(gOutputFilePrefix, savePath, 20, mode, gCaptureCnt, 500, setPDValue)
...
def WritePDValueLog(freq, currentmA, pdvalue, valuect, valuelt, valuert, valuerb, valuelb):
    global gSaveModulePath
    filePath = r'{0}\log.txt'.format(gSaveModulePath)
    strmsg = f'%s,%s,%s,%s,%s,%s,%s,%s\n' % (freq, currentmA, pdvalue, valuect, valuelt, valuert, valuerb, valuelb)
    f = open(filePath, 'a')
    f.write(strmsg)
    f.close
...
if __name__ == "__main__":
    # 관리자 권한으로 실행했는지 확인함*********************************************************************************
    msg_print(f'관리자 권한으로 실행했는지 확인합니다.')
    if not winsys.is_admin():
        print('[ERROR] You should run this program with administrator')
        sys.exit(-1)
    ...
    for i in list(range(0, 2800, 100)):
        pdValue = APCCheck(freq, i)  # 0x00
        ProcSaveRaw100mhz50cm(1, i)  # 1 -> only measurement
        WritePDValueLog(freq, i, pdValue, gIntecsityCT, gIntecsityLT, gIntecsityRT, gIntecsityRB, gIntecsityLB)
    ...
    for i in list(range(0, 2800, 100)):
        pdValue = APCCheck(freq, i)  # 0x00
        ProcSaveRaw20mhz50cm(1, i)  # 1 -> only measurement
        WritePDValueLog(freq, i, pdValue, gIntecsityCT, gIntecsityLT, gIntecsityRT, gIntecsityRB, gIntecsityLB)
