...
import struct
import python_simmian_api
from measurement import embedded_data_parser
...
gCaptureResult=False
gget_module_number=f''
#####################################################
...
def SetRegisterPD(register):
    ...
    WriteCmd(0x20, 0x2000E3A0, 0x0001)  # number of write bytes
    WriteCmd(0x20, 0x2000E3A2, 0x000E)  # register address of VCSEL Driver
    WriteCmd(0x20, 0x2000E3A4, register_code)  # data for 0x08
    WriteCmd(0x20, 0x2000E3A6, 0x00FF)  # Delay command
    WriteCmd(0x20, 0x2000E3A8, 0x000F)  # Data for Delay, 13.3us
    WriteCmd(0x20, 0x2000E3AA, 0x0000)  # Terminates command array
    WriteCmd(0x20, 0x2000E3E0, 0x0001)  # data at end address
...
def find_judge(mode, freq, data, indexCount):
    ...
    if freq == 100:
        if gApc_check_at_lsw_fix1_f1 >= 335 and gApc_check_at_lsw_fix1_f1 <= 735:
            apc_lsw_fix1 = True
        else:
            apc_lsw_fix1 = False
        ...
        if gApc_check_at_lsw_fix2_f1 >= 200 and gApc_check_at_lsw_fix2_f1 <= 470:
            apc_lsw_fix2 = True
        else:
            apc_lsw_fix2 = False
        ...
        if gApc_check_at_lsw_fix1_f2 >= 400 and gApc_check_at_lsw_fix1_f2 <= 750:
            apc_lsw_fix1 = True
        else:
            apc_lsw_fix1 = False
        ...
        if gApc_check_at_lsw_fix2_f2 >= 250 and gApc_check_at_lsw_fix2_f2 <= 500:
            apc_lsw_fix2 = True
        else:
            apc_lsw_fix2 = False
    ...
    df1 = pd.DataFrame(
        [
            [f1_0,
             ...
             gApc_check_at_lsw_fix2_f1,gCaptureResult],
            [f2_0,
             JudgeResult_f2[0],
             ...
             gApc_check_at_lsw_fix2_f2,datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f')]
        ],
        ...
        columns=['Judge',
                 ...
                 'board_offset f1/f2',
                 'APC Check 2700mA',
                 'APC Check 1500mA','time'
                 ])

    #file_path = r'{0}\{1}.csv'.format(Path, gSensor_id)  # file_path = r'{0}\{1}.csv'.format(Path, f'result')
    file_path = r'{0}\{1}.csv'.format(Path, "")  # file_path = r'{0}\{1}.csv'.format(Path, f'result')
...
def Convert_RawToCSV(filePath, folderPath, prefix, countIndex, frequency, cellLine, position,result_saver):
    ...
    fullpath = os.path.join(path, depth_file)
    emb=np.fromfile(fullpath, dtype='>B')
    data_paresr = embedded_data_parser.EmbeddedDataParser('63D', 'depth')
    my_data = data_paresr.parse_embedded_data(emb)

    frame_data=list()
    frame_data.append(depth[112:127, 152:167].mean())#depth ct mean
    frame_data.append(intensity[112:127, 152:167].mean())#intensity ct mean
    frame_data.append(my_data['Tx temperature'])
    frame_data.append(my_data['Rx temperature'])
    frame_data.append(datetime.datetime.now())
    result_saver.append(frame_data)


    # 뎁스 오차, 노이즈, 세기 대표 값 계산
    if position == 300:  # 측정 위치 300 mm
        # Depth Error
        gResult[(countIndex + ((cellLine - 1) * gCaptureCnt)), 0] = depth[112:127, 152:167].mean()  # CT
    else:  # 측정 위치 500 mm
        # Depth Error
...
def validation(prefix, path, frequency, mode, count, pos, result_saver):
    global gCaptureResult
    ...
    for i in range(0, count):
        ...
        # msg_print(filePath)
        if frequency == 100 and gCaptureResult:
            Convert_RawToCSV(filePath, path, prefix, i, frequency, 1, pos,result_saver)
            # RAW경로, 폴더경로, 파일이름, 파일이름순서, 동작주파수, , 측정위치 순이다.
        elif frequency == 20 and gCaptureResult:
            Convert_RawToCSV(filePath, path, prefix, i, frequency, 2, pos,result_saver)
            # RAW경로, 폴더경로, 파일이름, 파일이름순서, 동작주파수, , 측정위치 순이다.
        else:
            result_saver=None
            print('')
...
def ProcCreateModuleFolder(input_str=''):
    ...
    global gget_module_number
    gget_module_number= get_module_number
...
def ProcLoadSetFile100mhz():
    ...
    if g100MHzSetFilePath==' ':
        g100MHzSetFilePath=f'%s\%s' % (gSaveModulePath, g100MHzSetFileName)
...
def ProcLoadSetFile20mhz():
    ...
    msg_print(f'모듈에 맞는 새로 생성한 셋파일 경로 불러오기 **********************************************************')
    if g20MHzSetFilePath==' ':
        g20MHzSetFilePath=f'%s\%s' % (gSaveModulePath, g20MHzSetFileName)
...
if __name__ == "__main__":
    ...
    while frame_number <= end_frame:
        ...
        Measurement(100, 500, motion_dist500)  # 100MHz 500 mm 에서 평가
        # Measurement(20, 500, motion_dist500)  # 20MHz 500 mm 에서 평가
        ProcCreateReport(1)  # 0 -> find global offset , 1 -> only measurement
        msg_print(frame_number)
        frame_number+=1 #240202추가
