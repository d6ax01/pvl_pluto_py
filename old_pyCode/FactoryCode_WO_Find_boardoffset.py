...
def RetryAPCCheck(Minspec, Maxspec):
    i = 0
    while True:  # 무한 루프
        pd_Value = ReadPdValue()  # value = ReadPdValue()
        i += 1
        if Minspec <= pd_Value and pd_Value <= Maxspec:
...
def validation(prefix, path, frequency, mode, count, pos):
    ...
    for i in range(0, count):
        ...
        result = simmian.Capture(filePath, captureCount=1)
        delayMicroseconds(100000)  # 0.5초 #time.sleep(0.5)
        # msg_print(filePath)
        if frequency == 100:
...
def InitGlobalOffsetWriteI2C():
    ...
    set_board_offset_f1 = 16931  # 31560  # 계산한 옵셋이 0xabcd 이라면, WriteI2C 에 입력되는 값은 0xcdab 로 넣어야한다.
    ...
    set_board_offset_f2 = 49161  # 51521  # 계산한 옵셋이 0xabcd 이라면, WriteI2C 에 입력되는 값은 0xcdab 로 넣어야한다.
...
if __name__ == "__main__":
    ...
    if (want_home_position_at_start):
        ...
        motor_move(distance=500, motion_abs=402.2)  # 2023_01_13, 500mm(397.8)->506.6mm(402.2)
    ...
    # 모터 제어 : 위치 이동 300 mm *************************************************************************************
    motor_move(distance=300, motion_abs=199.5)  # 2023_01_13, 300mm(195.5)->306.6mm(199.5)
