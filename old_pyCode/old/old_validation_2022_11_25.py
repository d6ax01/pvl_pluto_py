...
def motor_open(want_home_position_at_start, motion_obj):
...
def ProcMoveTo500():
    ...
    # motion_obj = motion.Motion()  # 모터 초기화 명령어
    # gMotor = motor_open(False, motion_obj)  # 모터 오픈

    # if gMotor.ABSMove_Chk(motion.axis_no, 356.0) == False:  # 500mm -> 356.0 , 300mm -> 153.0
    #    print(f' => Oops: Motor could not move')

    ## 500 mm (absPos 356.0) 으로 이동
    # gMotor.MoveAbsolute(motion.axis_no, 356.0)
...
def ProcMoveto300():
    ...
    # 300 mm (absPos 153.0) 으로 이동
    # gMotor.MoveAbsolute(motion.axis_no, 153.0)
    # wait(lambda: is_motor_ready(gMotor, motion.axis_no), timeout_seconds=120, waiting_for="Motor move to be ready")
