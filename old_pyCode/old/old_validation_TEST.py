...
def InitGlobalOffsetWriteI2C():
    ...
    set_board_offset_f1 = 4267  # 계산한 옵셋이 0xabcd 이라면, WriteI2C 에 입력되는 값은 0xcdab 로 넣어야한다.
    set_board_offset_f1_ = TestReversed(set_board_offset_f1)
    str = (
    ...
    set_board_offset_f2 = 8397  # 계산한 옵셋이 0xabcd 이라면, WriteI2C 에 입력되는 값은 0xcdab 로 넣어야한다.
    set_board_offset_f2_ = TestReversed(set_board_offset_f2)
    str = (
...
if __name__ == "__main__":
    ...
    re_read_board_offset_f1 = simmian.ReadI2C(gDeviceID, gStartAddress_f1, 2)
    str = (f'board offset f1 : [E2P에서 읽어온 값] [셋파일 값]-> ', hex(re_read_board_offset_f1), HEXLittleEndian(re_read_board_offset_f1) )
    msg_print(str)

    re_read_board_offset_f2 = simmian.ReadI2C(gDeviceID, gStartAddress_f2, 2)
    str = (f'board offset f2 : [E2P에서 읽어온 값] [셋파일 값]-> ', hex(re_read_board_offset_f2), HEXLittleEndian(re_read_board_offset_f2))
    msg_print(str)

    new_gApc_check_at_lsw_fix1_f1 = simmian.ReadI2C(0xA0, "0x00d2", 2)
    time.sleep(0.010)  # 10ms
    str = (f'Apc_check_at_lsw_fix1_f1 [E2P에서 읽어온 값] [빅엔디안] -> ', hex(new_gApc_check_at_lsw_fix1_f1) , HEXLittleEndian(new_gApc_check_at_lsw_fix1_f1))
    msg_print(str)

    new_gApc_check_at_lsw_fix2_f1 = simmian.ReadI2C(0xA0, "0x00d4", 2)
    time.sleep(0.010)  # 10ms
    str = (f'Apc_check_at_lsw_fix2_f1 [E2P에서 읽어온 값] [빅엔디안] -> ', hex(new_gApc_check_at_lsw_fix2_f1) , HEXLittleEndian(new_gApc_check_at_lsw_fix2_f1))
    msg_print(str)

    new_gApc_check_at_lsw_fix1_f2 = simmian.ReadI2C(0xA0, "0x00d6", 2)
    time.sleep(0.010)  # 10ms
    str = (f'Apc_check_at_lsw_fix1_f2 [E2P에서 읽어온 값] [빅엔디안] -> ', hex(new_gApc_check_at_lsw_fix1_f2) , HEXLittleEndian(new_gApc_check_at_lsw_fix1_f2))
    msg_print(str)

    new_gApc_check_at_lsw_fix2_f2 = simmian.ReadI2C(0xA0, "0x00d8", 2)
    time.sleep(0.010)  # 10ms
    str = (f'Apc_check_at_lsw_fix2_f2 [E2P에서 읽어온 값] [빅엔디안] -> ', hex(new_gApc_check_at_lsw_fix2_f2) , HEXLittleEndian(new_gApc_check_at_lsw_fix2_f2))
    msg_print(str)
    ...
    # RAW 저장 100MHz (위치 : 500 mm ) *********************************************************************************
    time.sleep(3)  # 성능평가를 위한 뎁스 영상 취득 전에 워밍업 3초
    ProcSaveRaw100mhz50cm(0)  # 0 -> find global offset
