...
def find_judge(mode, freq, data, indexCount):
    ...
    if freq == 100:
        ...
        ConfidenceAVG_Spec_CenterUpper = 2188;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> Center ROI (상한선)
        ConfidenceAVG_Spec_06FieldLower = 625;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> 0.6F ROI (하한선)
        ConfidenceAVG_Spec_06FieldUpper = 1563;  # //500mm VCSEL Optical Power Data_f1 (광량검사 데이터) -> 0.6F ROI (상한선)
    else:  # 20MHz
        # // 검사 사양 : 20MHz, 500mm VCSEL Optical Power Data_f2 (광량검사 데이터)
        ConfidenceAVG_Spec_CenterLower = 625;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> Center ROI (하한선)
        ConfidenceAVG_Spec_CenterUpper = 1563;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> Center ROI (상한선)
        ConfidenceAVG_Spec_06FieldLower = 938;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> 0.6F ROI (하한선)
        ConfidenceAVG_Spec_06FieldUpper = 2188;  # //500mm VCSEL Optical Power Data_f2 (광량검사 데이터) -> 0.6F ROI (상한선)
    ...
    if freq == 100:
        ...
        if gApc_check_at_lsw_fix1_f2 >= 600 and gApc_check_at_lsw_fix1_f2 <= 1023:
            apc_lsw_fix1 = True
        else:
            apc_lsw_fix1 = False
...
if __name__ == "__main__":
    ...
    if (want_home_position_at_start):
        ...
        motor_move(distance=500, motion_abs=404.2)  # 2023_01_13, 500mm(397.8)->506.6mm(404.2)
        #motor_move(distance='loading', motion_abs=78)  # isMedia Motor
        time.sleep(0.5)
    ...
    # 모터 제어 : 위치 이동 300 mm *************************************************************************************
    motor_move(distance=300, motion_abs=200.5)  # 2023_01_13, 300mm(195.5)->306.6mm(200.5)
