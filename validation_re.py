...
#101 IS_ADMIN
#102 IS_MOTOR
#103 OTP_WRITE -> e2p 쓰기 실패
#104 -> cal 리니어 브로큰 에러
#105 -> cal thr??? 에러
#106 -> 파일갯수부족
#107 -> IS_CONNECT_CHANGE
...
def APCCheck(freq):
    ...
    SetRegisterPD(setregvalue)  # R_PD = 0b100:open , # 브링업 투식스 TX사용 시 적용
...
def judgecode(code, msg):
    ...
    if is_code == 0:
        messagebox(msg)
        btnStart.configure(text='PASS', font=("", 46), bg="Green", fg="Black", overrelief="solid")

    else:
        messagebox(msg)
        btnStart.configure(text='FAIL', font=("", 46), bg="Red", fg="White",overrelief="solid")
...
def kill_nxsimmian():
    for proc in psutil.process_iter():
        if proc.name() == "SimmianApp.exe":
            ...
            print(f'Can not find SimmianApp.exe')
...
def runsimmianstatus():
    re = ctypes.windll.user32.MessageBoxW(0, f'Is it run NxSimmian Viewer?', " ", 52) # re -> 6 YES , 7 -> No
...
def btnStartCal() :
    btnStart.configure(text='Run', font=("", 46), bg="Cyan", fg="White", overrelief="solid")
    ...
    # E2P 쓰기 잠금해제 unlock *******************************************
    re_romlock = eepromlock(0)  # 0 -> unlock , 1 -> lock
    if re_romlock == 0:
...
def btnStartVal() :
    ...
    # validation 평가 시작 ***********************************************
    if re_romlock == 0:
        re_val = validationproc(re_sim)
        if re_val == 0:  # 0 -> PASS
            str = f'[{re_val}] PASS -> Validation'
            messagebox(re_val)
        else:
            str = f'[{re_val}] FAIL -> Validation'
            judgecode(re_val, str)
            return re_val
    ...
    # 결과 ************************************************************
    if re_val == 0 :
        str = f'[{0}] PASS -> Validation was done.'
        judgecode(0, str)
    else:
        str = f'[{1}] FAIL -> Validation was not done.'
        judgecode(1, str)
...
def init(mode) :
    ...
    # create button ->
    if mode == cal_mode:
        btnStart = Button(root, text='Ready', font=("", 46),overrelief="solid", command=btnStartCal)
        btnStart.place(x=10, y=10)  # 버튼 위치
    elif mode == val_mode:
        btnStart = Button(root, text='Ready', font=("", 46),overrelief="solid", command=btnStartVal)
        btnStart.place(x=10, y=10)  # 버튼 위치
    else:
        exit()
    ...
    # 리스트 박스 생성
    listbox = Listbox(root, width=51, height=28)
